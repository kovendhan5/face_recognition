...
def display_dashboard(attendance_df, student_df):
    ...
    st.markdown("<h1 style='text-align: center; color: #4CAF50; font-family: Arial, sans-serif;'>📋 Attendance Dashboard for Staff</h1>", unsafe_allow_html=True)
...
# Display dashboard
def display_dashboard(attendance_df, student_df):
    st.markdown("<h1 style='text-align: center; color: #4CAF50;'>📋 Attendance Dashboard for Staff</h1>", unsafe_allow_html=True)
    unique_dates = attendance_df['Date'].nunique()
    
    # Ensure 'Name' column is available after merging
    attendance_with_dept = attendance_df.merge(student_df[['Roll', 'Name', 'Department']], on='Roll', how='left')
    
    # If 'Name' column is still missing, set total_attendees to 0 to avoid errors
    if 'Name' in attendance_with_dept.columns:
        total_attendees = attendance_with_dept['Name'].nunique()
        total_present = attendance_with_dept[attendance_with_dept['Time'].notnull()]['Name'].nunique()
    else:
        total_attendees = 0
    ...
    # Attendance Summary Chart by Department
    st.markdown("### Attendance Summary by Department")
    if 'Name' in attendance_with_dept.columns:
        summary_chart = attendance_with_dept.groupby('Department')['Name'].nunique().reset_index()
        summary_chart.columns = ['Department', 'Unique Students']
        summary_fig = px.bar(summary_chart, x='Department', y='Unique Students', title="Unique Students by Department", color='Department')
        ...
        st.markdown("No student name data available for the summary chart.")
